      ...
      function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
      {
           ...
           if($depth != 0)
           {
            ...
            if (!empty($item->color)) 
                $item_output .= '';
            $item_output .= '>';
            $item_output .= ' <style>#menu-item-'. $item->ID . ' a, .menu-item-'. $item->ID . '-megamenu, #menu-item-'. $item->ID . ' .sub-menu { border-color: ' . $item->color . ' !important; } 
#menu-item-'. $item->ID . ' a:hover, #wpmm-megamenu.menu-item-'. $item->ID . '-megamenu a:hover, #wpmm-megamenu.menu-item-'. $item->ID . '-megamenu .wpmm-posts .wpmm-entry-title a:hover { color: '.$item->color.' !important; }</style>';
            // Icon
            if (!empty($item->icon)) {
